// Cleaned main.js — single copy, modal cleanup and deduped notifications
(function(){
  // --- Small helpers ---
  ...
  // seed admin (intended behaviour)
  localStorage.setItem(LS_ADMIN, JSON.stringify(DEFAULT_ADMIN));

  // Simple single-instance toast to avoid duplicate alert boxes
  let _lastToast = {msg:'', ts:0};
  function showToast(msg, timeout = 1800){
    try{
      const now = Date.now();
      if(msg === _lastToast.msg && now - _lastToast.ts < 1200) return; // dedupe identical messages quickly
      _lastToast = {msg, ts: now};
      const containerId = 'mxz-toast-container';
      let container = document.getElementById(containerId);
      if(!container){ container = document.createElement('div'); container.id = containerId; container.style.position='fixed'; container.style.right='18px'; container.style.bottom='18px'; container.style.zIndex='10800'; document.body.appendChild(container); }
      const el = document.createElement('div');
      el.className = 'mxz-toast card p-2 mb-2';
      el.style.minWidth = '220px'; el.style.background = 'rgba(30,30,30,0.95)'; el.style.color = '#fff'; el.style.boxShadow = '0 6px 18px rgba(0,0,0,0.4)';
      el.textContent = msg;
  ...
  // Optional Supabase init (unchanged)
  ...
  let useSupabase = false, supabaseClient = null;
  if(SUPABASE_URL && SUPABASE_KEY && window.supabase && typeof window.supabase.createClient === 'function'){
    try{ supabaseClient = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY); useSupabase = true; }catch(e){console.warn('Supabase init failed', e)}
  }
  ...
  function setLoggedIn(u){
    ...
    document.querySelectorAll('.admin-only').forEach(btn => btn.classList.toggle('d-none', !isLoggedIn));
    ...
    try { if (typeof renderExperiences === 'function') renderExperiences(); } catch (e) {}
  ...
  if(loginBtnEl){
    loginBtnEl.addEventListener('click', ()=>{
      if(isLoggedIn){
        if(confirm('Logout?')){ setLoggedIn(false); sessionStorage.removeItem('portfolio_session'); showToast('Logout berhasil'); }
      } else {
  ...
  if(loginForm){
    loginForm.addEventListener('submit', e=>{
      ...
      if(u === admin.username && p === admin.password){
        ...
        setLoggedIn(u);
        const inst = bootstrap.Modal.getInstance(document.getElementById('loginModal'));
  ...
  // --- Projects, About, Skills, Experience (kept same behaviour, but single copy only) ---
  function getProjects(){ try{ return JSON.parse(localStorage.getItem(LS_PROJECTS)) || [] }catch(e){return []} }
  function saveProjects(arr){ localStorage.setItem(LS_PROJECTS, JSON.stringify(arr)); try{ renderProjects(); }catch(e){} if(useSupabase && supabaseClient){ (async ()=>{ try{ const payload = arr.map(p=>({ id: p.id || Date.now().toString(), title:p.title, description:p.desc, image_url:p.image, link:p.link, created_at:p.created_at || new Date().toISOString() })); await supabaseClient.from('projects').upsert(payload, {onConflict:'id'}); }catch(e){ console.warn('Supabase upsert failed', e); } })(); } }

  const projectForm = document.getElementById('projectForm');
  if(projectForm){ projectForm.addEventListener('submit', e=>{ e.preventDefault(); if(!isLoggedIn) return showToast('Hanya admin yang bisa menambah proyek.'); const id = document.getElementById('projId').value || Date.now().toString(); const proj = {id, title:document.getElementById('projTitle').value, image:document.getElementById('projImage').value, link:document.getElementById('projLink').value, desc:document.getElementById('projDesc').value }; const arr = getProjects(); const idx = arr.findIndex(p=>p.id===id); if(idx>=0) arr[idx]=proj; else arr.unshift(proj); saveProjects(arr); try{ bootstrap.Modal.getInstance(document.getElementById('projectModal'))?.hide(); cleanupModalArtifacts(); }catch(e){} }); }

  function renderProjects(){ const grid = document.getElementById('projectsGrid'); if(!grid) return; grid.innerHTML=''; const arr = getProjects(); if(arr.length===0) grid.innerHTML = '<div class="col-12 text-center text-muted">Belum ada proyek. Login sebagai admin untuk menambah.</div>'; arr.forEach(p=>{ const col = document.createElement('div'); col.className='col-md-6 col-lg-4'; col.innerHTML = `
  ...
  if(projectsGrid){ projectsGrid.addEventListener('click', e=>{ const btn = e.target.closest('button'); if(!btn) return; const id = btn.dataset.id; const action = btn.dataset.action; if(action==='edit'){ const proj = getProjects().find(x=>x.id===id); if(!proj) return; document.getElementById('projId').value = proj.id; document.getElementById('projTitle').value = proj.title; document.getElementById('projImage').value = proj.image; document.getElementById('projLink').value = proj.link; document.getElementById('projDesc').value = proj.desc; try{ new bootstrap.Modal(document.getElementById('projectModal')).show(); }catch(e){} } else if(action==='del'){ if(!confirm('Hapus proyek ini?')) return; const arr = getProjects().filter(x=>x.id!==id); saveProjects(arr); } }); }

  // Contact form (mailto)
  const contactForm = document.getElementById('contactForm'); if(contactForm){ contactForm.addEventListener('submit', e=>{ e.preventDefault(); const name = encodeURIComponent(document.getElementById('contactName').value); const email = encodeURIComponent(document.getElementById('contactEmail').value); const msg = encodeURIComponent(document.getElementById('contactMessage').value); const to = document.getElementById('emailLink').textContent || 'your.email@example.com'; window.location.href = `mailto:${to}?subject=${encodeURIComponent('Pesan dari '+decodeURIComponent(name))}&body=${msg}%0A%0A--%0A${email}`; }); }

  if(getProjects().length===0){ const sample = [ {id:'p1', title:'Todo App', image:'https://picsum.photos/seed/todo/800/600', link:'#', desc:'Aplikasi todo dengan localStorage.'}, {id:'p2', title:'Portfolio Website', image:'https://picsum.photos/seed/port/800/600', link:'#', desc:'Website pribadi yang menampilkan proyek dan kontak.'} ]; localStorage.setItem(LS_PROJECTS, JSON.stringify(sample)); }
  renderProjects();

  // skills & experiences (kept concise)
  const btnAddSkill = document.getElementById('btnAddSkill'); const skillFormEl = document.getElementById('skillForm'); const skillInputEl = document.getElementById('skillInput'); const skillsListEl = document.getElementById('skillsList');
  function getSkills(){ try{ return JSON.parse(localStorage.getItem(LS_SKILLS)) || ['HTML','CSS','JavaScript','Bootstrap','Python'] }catch(e){return ['HTML','CSS','JavaScript','Bootstrap','Python']} }
  function renderSkills(){ if(!skillsListEl) return; skillsListEl.innerHTML = ''; const skills = getSkills(); skills.forEach(skill=>{ const span = document.createElement('span'); span.className = 'badge skill-badge p-2 position-relative'; span.textContent = skill; if(isLoggedIn){ span.style.cursor = 'pointer'; span.title = 'Klik untuk hapus'; span.addEventListener('click', ()=>{ if(confirm(`Hapus skill "${skill}"?`)){ const updated = getSkills().filter(s=>s!==skill); localStorage.setItem(LS_SKILLS, JSON.stringify(updated)); renderSkills(); if(useSupabase && supabaseClient){ (async ()=>{ try{ await supabaseClient.from('skills').delete().eq('name', skill); }catch(e){ console.warn('Supabase delete skill failed', e); } })(); } } }); } skillsListEl.appendChild(span); }); }
  if(btnAddSkill) btnAddSkill.addEventListener('click', ()=>{ if(skillInputEl) skillInputEl.value = ''; });
  if(skillFormEl){ skillFormEl.addEventListener('submit', e=>{ e.preventDefault(); if(!isLoggedIn) return showToast('Hanya admin yang bisa tambah skill'); const skill = skillInputEl.value.trim(); if(skill){ const skills = getSkills(); if(!skills.includes(skill)){ skills.push(skill); localStorage.setItem(LS_SKILLS, JSON.stringify(skills)); if(useSupabase && supabaseClient){ (async ()=>{ try{ await supabaseClient.from('skills').insert({name:skill}); }catch(e){ console.warn('Supabase insert skill failed', e); } })(); } renderSkills(); skillInputEl.value = ''; try{ bootstrap.Modal.getInstance(document.getElementById('skillModal'))?.hide(); cleanupModalArtifacts(); }catch(e){} } else showToast('Skill sudah ada'); } }); }
  renderSkills();

  // experiences
  const btnAddExpEl = document.getElementById('btnAddExp'); const expFormEl = document.getElementById('expForm'); const expListEl = document.getElementById('expList');
  function getExperiences(){ try{ return JSON.parse(localStorage.getItem(LS_EXPERIENCE)) || [{id:'exp1',title:'Intern Web Developer',company:'Perusahaan X, 2024',desc:'Deskripsi singkat pengalaman kerja atau tugas yang dilakukan.'}] }catch(e){return []} }
  function renderExperiences(){ if(!expListEl) return; expListEl.innerHTML = ''; const exps = getExperiences(); if(exps.length === 0) expListEl.innerHTML = '<div class="text-muted">Belum ada pengalaman. Login sebagai admin untuk menambah.</div>'; exps.forEach(exp=>{ const li = document.createElement('li'); li.className = 'mb-3 card card-dark p-3'; li.innerHTML = `
  ...
  if(btnAddExpEl) btnAddExpEl.addEventListener('click', ()=>{ document.getElementById('expId').value = ''; document.getElementById('expTitle').value = ''; document.getElementById('expCompany').value = ''; document.getElementById('expDesc').value = ''; });
  if(expFormEl){ expFormEl.addEventListener('submit', e=>{ e.preventDefault(); if(!isLoggedIn) return showToast('Hanya admin yang bisa tambah pengalaman'); const id = document.getElementById('expId').value || Date.now().toString(); const exp = {id, title:document.getElementById('expTitle').value, company:document.getElementById('expCompany').value, desc:document.getElementById('expDesc').value}; const exps = getExperiences(); const idx = exps.findIndex(x=>x.id===id); if(idx>=0) exps[idx]=exp; else exps.unshift(exp); localStorage.setItem(LS_EXPERIENCE, JSON.stringify(exps)); renderExperiences(); if(useSupabase && supabaseClient){ (async ()=>{ try{ await supabaseClient.from('experiences').upsert({id: exp.id, title: exp.title, company: exp.company, description: exp.desc, created_at: new Date().toISOString() }, {onConflict:'id'}); }catch(e){ console.warn('Supabase experiences upsert failed', e); } })(); } try{ bootstrap.Modal.getInstance(document.getElementById('expModal'))?.hide(); cleanupModalArtifacts(); }catch(e){} }); }
  if(expListEl){ expListEl.addEventListener('click', e=>{ const btn = e.target.closest('button'); if(!btn) return; const id = btn.dataset.id; const action = btn.dataset.action; if(action==='edit-exp'){ const exp = getExperiences().find(x=>x.id===id); if(!exp) return; document.getElementById('expId').value = exp.id; document.getElementById('expTitle').value = exp.title; document.getElementById('expCompany').value = exp.company; document.getElementById('expDesc').value = exp.desc; try{ new bootstrap.Modal(document.getElementById('expModal')).show(); }catch(e){} } else if(action==='del-exp'){ if(!confirm('Hapus pengalaman ini?')) return; const exps = getExperiences().filter(x=>x.id!==id); localStorage.setItem(LS_EXPERIENCE, JSON.stringify(exps)); renderExperiences(); if(useSupabase && supabaseClient){ (async ()=>{ try{ await supabaseClient.from('experiences').delete().eq('id', id); }catch(e){ console.warn('Supabase delete experience failed', e); } })(); } } }); }
  renderExperiences();

  // UI enhancements -- reveal, typing, tilt (kept as before but single copy)
  (function uiEnhance(){
    try{ const nodes = Array.from(document.querySelectorAll('.reveal')); const obs = new IntersectionObserver((entries,o)=>{ entries.forEach(en=>{ if(en.isIntersecting){ en.target.classList.add('visible'); o.unobserve(en.target); } }); },{threshold:0.12}); nodes.forEach(n=>obs.observe(n)); }catch(e){}
    try{ const hero = document.querySelector('.hero-typed'); if(hero){ const rawWords = (hero.dataset.words || '').split('|').filter(Boolean); const words = rawWords.length ? rawWords : ['Full-Stack Developer','Web Developer','AI Enthusiast']; let wIndex=0,chIndex=0,forward=true; if(!hero.childNodes[0] || hero.childNodes[0].nodeType !== Node.TEXT_NODE) hero.insertBefore(document.createTextNode(''), hero.firstChild); const textNode = hero.childNodes[0]; function tick(){ const w = words[wIndex]; if(forward){ chIndex++; textNode.nodeValue = w.slice(0,chIndex); if(chIndex>=w.length){ forward=false; setTimeout(tick,1200); return; } } else { chIndex--; textNode.nodeValue = w.slice(0,chIndex); if(chIndex<=0){ forward=true; wIndex=(wIndex+1)%words.length; setTimeout(tick,300); return; } } setTimeout(tick, forward?70:30); } tick(); } }catch(e){}
    try{ const cards = document.querySelectorAll('.card'); cards.forEach(card=>{ card.classList.add('card-tilt'); let ticking=false,lastEvent=null; card.addEventListener('mousemove', e=>{ lastEvent=e; if(!ticking){ ticking=true; requestAnimationFrame(()=>{ const rect = card.getBoundingClientRect(); const cx = rect.left + rect.width/2; const cy = rect.top + rect.height/2; const dx = lastEvent.clientX - cx; const dy = lastEvent.clientY - cy; const rx = (dy / rect.height) * -4; const ry = (dx / rect.width) * 6; card.style.transform = `perspective(800px) rotateX(${rx}deg) rotateY(${ry}deg)`; ticking=false; }); } }, {passive:true}); card.addEventListener('mouseleave', ()=>{ card.style.transform = ''; }); }); }catch(e){}
  })();
